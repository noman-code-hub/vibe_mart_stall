...
function drawProductOverlays(ctx, products, images) {
  POS.products.forEach((slot, index) => {
    ...
    const desc = product.variation || product.label || product.description || '';
    const price = product.price || '';
    const amount = String(price).replace(/£/g, '').trim();
    const priced = amount ? `${amount}£` : '';
    const combined = [name, desc, priced].filter(Boolean).join('  ');
